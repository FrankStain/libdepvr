...
namespace tga {
	const bool load( const string& path, image_desc_t& desc ){
		if( path.empty() || !file_system::file_exists( path ) ){
			return false;
		};
			...
			switch( hdr->m_image.m_bit_depth ){
				case 16:{
					...
					for( int32_t rd = 0; hdr->m_image.m_height > rd; rd++ ){
						pvr::color32_t* dest	= desc.row( rd );
						...
						for( uint16_t* pix = (uint16_t*)(pixels + rd * row_stride); eol > pix; pix++ ){
							...
							dest->m_red		= (pvr::channel_t)( 0.5f + ( 0x1FU & ( (*pix) >> 10 ) ) * expand_pct );
							dest->m_green	= (pvr::channel_t)( 0.5f + ( 0x1FU & ( (*pix) >> 5 ) ) * expand_pct );
							dest->m_blue		= (pvr::channel_t)( 0.5f + ( 0x1FU & (*pix) ) * expand_pct );
				...
				case 32:
					...
					pvr::color32_t* pix = desc.memory();
					const pvr::color32_t* eol = pix + desc.width() * desc.height();
					while( eol > pix ){
						const pvr::channel_t ch = pix->m_red;
	...
	const bool save( const string& path, const image_desc_t& desc, const uint16_t bit_depth, const bool save_aplha ){
		...
		switch( bit_depth ){
			case 16:{
				...
				pvr::color32_t* in_pix = desc.memory();
			...
			case 32:{
				pvr::color32_t* data = new pvr::color32_t[ data_size ];
				memcpy( data, desc.memory(), 4 * data_size );

				for( pvr::color32_t* pix = data; ( data + data_size ) > pix; pix++ ){
					const pvr::channel_t ch = pix->m_red;
					pix->m_red	= pix->m_blue;
					pix->m_blue	= ch;
